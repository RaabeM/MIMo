...
def test(env, test_for=1000, model=None):
    ...
    for idx in range(test_for):
        ...
        if TEST:
            ...
            saliencyMap_left = img_left
            saliencyMap_right= img_right
        ...
        #print(f"horizontal: {env.sim.data.qpos[16]}, vertical {env.sim.data.qpos[17]}, torsional {env.sim.data.qpos[18]}")

        target_pos = env.sim.data.get_body_xpos('target')
        left_eye_pos = env.sim.data.get_body_xpos('left_eye')
        radius_eye = 0.01125
        eye_target_dist = np.linalg.norm(target_pos - left_eye_pos) - radius_eye


        # TODO - Fix pointing to max saliency
        #env.sim.data.qpos[16] = np.arcsin((target_pos[1] - left_eye_pos[1])/eye_target_dist) + (128 - max_saliency_left[1][0]) * math.pi/(3*256) # left eye -  horizontal
        #env.sim.data.qpos[17] = np.arcsin(-(left_eye_pos[2]-target_pos[2])/eye_target_dist)  + (128 - max_saliency_left[0][0]) * math.pi/(3*256) # left eye -  vertical

        env.sim.data.qpos[16] +=  (128 - max_saliency_left[1][0]) * math.pi/(3*256) # left eye -  horizontal
        env.sim.data.qpos[17] +=  (128 - max_saliency_left[0][0]) * math.pi/(3*256) # left eye -  vertical     

        env.sim.data.qpos[18] = 0
        #env.sim.data.qpos[17] = 2*math.pi# left eye -  vertical
        

        #print(f"horizontal: {env.sim.data.qpos[16]}, vertical {env.sim.data.qpos[17]}, torsional {env.sim.data.qpos[18]}")
        #print(f"Target: {target_pos}")
        #print(f"eye:    {left_eye_pos}")
        #print(f"{env.vision.camera_parameters}")

        #test = env.utils.get_body_position(env.sim.data, 'left_eye')


        #print(f"{target_pos}, {left_eye_pos}")
...
def main():
    ...
    if algorithm is None:
        ...
        model = RL.load("models/saccade" + load_model, env)
    else:
